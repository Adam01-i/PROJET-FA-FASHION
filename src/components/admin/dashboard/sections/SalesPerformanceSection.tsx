...
import { TrendingUp, Download, ShoppingBag, DollarSign } from "lucide-react";
...
// Dans SalesPerformanceSection.tsx
export default function SalesPerformanceSection() {
  ...
  // Utiliser les hooks dynamiques
  const { dailySales, loading: dailySalesLoading } = useDailySales(timeRange);
  const { salesTrend, loading: trendLoading } = useSalesTrend();

  // Dans le composant, corriger le calcul du panier moyen
  // Dans SalesPerformanceSection.tsx - améliorer le calcul des métriques
  const performanceData = useMemo((): SalesPerformanceData => {
    // Commandes confirmées seulement
    ...
    const categoryRevenue = productSales.reduce((acc, sale) => {
      ...
      acc[category] = (acc[category] || 0) + sale.total_revenue;
      return acc;
    ...
    const totalOrdersRevenue = confirmedOrders.reduce(
      (sum, order) => sum + order.total_amount,
      0
    );
    const averageOrderValue =
      confirmedOrders.length > 0
        ? totalOrdersRevenue / confirmedOrders.length
        : 0;
  ...
  }, [productSales, orders, dailySales, salesTrend]);

  // Améliorer l'affichage du loading
  if (salesLoading || ordersLoading || dailySalesLoading || trendLoading) {
    return (
            ...
            <div className="flex items-end justify-between h-28 sm:h-32 lg:h-48">
              {performanceData.dailySales.length > 0 ? (
                performanceData.dailySales.slice(-7).map((day, index) => {
                  // Calcul dynamique des hauteurs max basé sur les données réelles
                  const maxRevenue = Math.max(
                    ...performanceData.dailySales.map((d) => d.revenue),
                    1
                  );
                  const maxOrders = Math.max(
                    ...performanceData.dailySales.map((d) => d.orders),
                    1
                  );
                      ...
                      <div className="flex items-end space-x-0.5 sm:space-x-1 h-16 sm:h-20 lg:h-32 w-full justify-center">
                        <div
                          ...
                          style={{
                            ...
                            minHeight: "4px", // Hauteur minimale pour voir la barre
                          }}
            ...
            <tbody>
              {productSales.slice(0, 10).map((product) => (
                ...
                >
                  ...
                  <td className="py-2 sm:py-3 px-2 sm:px-4 text-gray-900 font-medium text-xs sm:text-sm">
